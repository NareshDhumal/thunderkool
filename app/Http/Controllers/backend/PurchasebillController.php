...
class PurchasebillController extends Controller
{
    public function index()
    {
        ...
        $purchase_bill_details = Finalpurchasebill::with('company', 'supplier', 'Product_details')->orderBy('id', 'DESC')->get();
    ...
    public function store(request $request)
    {
        ...
        $this->validate($request, [
            ...
            'company_id' => 'required',
            'supplier_bill_no' => 'required|numeric|min:0',
            'dated' => 'required',
        ...
        if (isset($request->product_name)) {
            if ($final_purchase_bill->save()) {
                ...
                for ($i = 0; $i < count($request->product_name); $i++) {
                    ...
                    if ($request->unit[$i] == 'Nos') {

                        $product_data['quantity'] = $request->quantity[$i];
                        $product_data['stock'] = null;
                        $product_data['product_unit'] = $request->unit[$i];
                    } else if ($request->unit[$i] == 'Gm') {

                        $product_data['quantity'] = $request->grams[$i];
                        $product_data['stock'] = $request->quantity[$i];
                        $product_data['product_unit'] = $request->unit[$i];
                    } else if ($request->unit[$i] == 'Mil') {

                        $product_data['quantity'] = $request->grams[$i];
                        $product_data['stock'] = $request->quantity[$i];
                        $product_data['product_unit'] = $request->unit[$i];
                    } else if ($request->unit[$i] == 'Kg') {

                        $product_data['quantity'] = $request->grams[$i];
                        $product_data['stock'] = $request->quantity[$i];
                        // $product_data['stock'] = null;
                        $product_data['product_unit'] = $request->unit[$i];
                    } else if ($request->unit[$i] == 'Lit') {
                    ...
                    //commented by mahesh for product name
                    // }
                    // // dd($request->product_name);
                    // $r_grams = 0;
                    // for ($i = 0; $i < count($request->product_name); $i++) {
                    //commented by mahesh for product name end

                    $pro_q = Product::where('product_id', $request->product_name[$i])->first();

                    if (isset($pro_q->product_stock)) {

                        if ($request->unit[$i] == 'Nos' && $pro_q->product_unit == 'Nos') {

                            $product_quantity = (int)$pro_q->product_stock + (int)$request->quantity[$i];
                            $pro_q->product_stock = $product_quantity;

                        } else if ($request->unit[$i] == 'Gm' && $pro_q->product_unit == 'Gm') {

                            $product_quantity = (int)$pro_q->product_stock + (float)$request->grams[$i];

                            $pro_q->product_stock = $product_quantity;

                        } else if ($request->unit[$i] == 'Kg' && $pro_q->product_unit == 'Gm') {

                            $product_quantity = (int)$pro_q->product_stock + (float)$request->grams[$i] * 1000;
                            $pro_q->product_stock = $product_quantity;

                        } else if ($request->unit[$i] == 'Mil' && $pro_q->product_unit == 'Mil') {

                            $product_quantity = (int)$pro_q->product_stock + (int)$request->grams[$i];
                            $pro_q->product_stock = $product_quantity;

                        } else if ($request->unit[$i] == 'Lit' && $pro_q->product_unit == 'Mil') {

                            $product_quantity = (int)$pro_q->product_stock + (float)$request->grams[$i] * 1000;
                            $pro_q->product_stock = $product_quantity;
                        } else {
                            return redirect()->back()->with('error', 'Please Select The Correct Unit First');
                        }
                        ...
                        if ($request->unit[$i] == 'Nos') {
                            ...
                            $product_arr['product_stock'] = $request->quantity[$i];
                            $product_arr['product_unit'] = $request->unit[$i];
                        } else if ($request->unit[$i] == 'Gm') {

                            $product_arr['product_stock'] = $request->grams[$i];
                            $product_arr['product_unit'] = $request->unit[$i];
                        } else if ($request->unit[$i] == 'Mil') {
                            ...
                            $product_arr['product_stock'] = $request->grams[$i] * 1000;
                            $product_arr['product_unit'] = 'Gm';
                        } else if ($request->unit[$i] == 'Lit') {

                            $product_arr['product_stock'] = $request->grams[$i] * 1000;
                            $product_arr['product_unit'] = 'Mil';
                        }
                        ...
                        $purchase_bill->product_name = $m_product->product_id;
        ...
        if (isset($purchase_bill)) {
            ...
            if ($purchase_bill->save()) {
                ...
                foreach ($purchase_bill as $purchase) {
                    ...
                    $product = Product::where('product_id', $purchase->product_name)->get();
                    // dd($purchase->quantity);
                    $stock = 0;
                    foreach ($product as $products) {
                        // dd($products->product_unit);

                        if ($purchase->product_unit == 'Nos') {

                            $stock = (int)$products->product_stock - (int)$purchase->quantity;
                            $products->product_stock = $stock;
                        } elseif ($purchase->product_unit == 'Gm') {

                            $stock = (int)$products->product_stock - (int)$purchase->quantity;

                            $products->product_stock = $stock;
                        } elseif ($purchase->product_unit == 'Mil') {

                            $stock = (int)$products->product_stock - (int)$purchase->quantity;
                            $products->product_stock = $stock;
                        } elseif ($purchase->product_unit == 'Kg') {

                            $stock = (int)$products->product_stock - (float)$purchase->quantity * 1000;
                            $products->product_stock = $stock;
                        } elseif ($purchase->product_unit == 'Lit') {
                            $stock = (int)$products->product_stock - (float)$purchase->quantity * 1000;
                            $products->product_stock = $stock;
                        }
                        // $stock = (int)$products->product_stock - (int)$purchase->quantity;
                ...
                for ($i = 0; $i < count($request->product_name); $i++) {
                    ...
                    //commented by mahesh sir for product name

                    // }
                    // for ($i = 0; $i < count($request->product_name); $i++) {

                    //commented by mahesh sir for product name end
    ...
    public function productdetails(Request $request)
    {
        ...
        $product = Product::where(['company_id' => $request->select_val,'service_id' => '2'])->pluck('product_name', 'product_id');
    ...
    public function producthsn(Request $request)
    {
        ...
        $product = Product::where('product_id', $request->select2_val)->first(['hsn_code', 'product_rate', 'product_unit']);
        // return $product;
    ...
    public function getproducts(Request $request)
    {
        ...
        $products = Product::where('product_id', $request->product_name)->first();
        if ($products->product_id  == $request->product_name) {
            $data = $products;
        }
        return $data;
    }
