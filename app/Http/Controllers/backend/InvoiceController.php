...
class InvoiceController extends Controller
{
            ...
            if ($request->product_counter > 0) {
                ...
                if (isset($request->product_row_counter)) {
                    ...
                    foreach ($request->product_row_counter as $product_row_count) {
                        ...
                        $data += $productInvoice->cgst_amount + $productInvoice->sgst_amount + $productInvoice->igst_amount;
        ...
        if ($request->gst_flag == 0) {
            ...
            if ($request->row_count > 0) {
                ...
                for ($i = 0; $i < $request->row_count; $i++) {
                    ...
                    $productInvoice->product_total_amount = $request->$product_total_amount;
                    $productInvoice->save();
                    // dd($productInvoice->cgst_amount, $productInvoice->sgst_amount, $productInvoice->igst_amount);

                    $total_gst_amount += $productInvoice->cgst_amount + $productInvoice->sgst_amount + $productInvoice->igst_amount;
                }
            ...
            foreach ($productInvoice as $data) {
                ...
                if ($product) {
                    ...
                    if ($data->p_unit == '1') {
                        ...
                        $product->product_stock = (int)$product->product_stock + (float)$data->quantity * 1000;
                    } else if ($data->p_unit == '2') {
                        $product->product_stock = (int)$product->product_stock + (float)$data->quantity * 1000;
                    }
                    // } 
    ...
    public function multiview()
    {
        ...
        return view('backend.invoice.multicopy');
    }
}
